    ...
    static getRandomContent(dictionary: any[][], config: DailyContentConfig = { 
        ...
        if (type === 'idiom') {
            const idioms = dictionary.filter(row => {
                const t = (row[AppConfig.COLUMNS.TYPE] || '').toLowerCase();
            ...
            if (idioms.length > 0) {
                ...
                return {
                    type: 'idiom',
                    id: item[AppConfig.COLUMNS.ID],
                    swedish: item[AppConfig.COLUMNS.SWEDISH],
                    translation: item[AppConfig.COLUMNS.ARABIC],
                    example: item[AppConfig.COLUMNS.EXAMPLE_SWE] || item[AppConfig.COLUMNS.DEFINITION],
                    tags: ['Idiom', 'تعبير']
                };
            }
        }
        ...
        const richWords = dictionary.filter(row => {
            const ex1 = (row[AppConfig.COLUMNS.EXAMPLE_SWE] || '').trim();
        ...
        return {
            ...
            swedish: item[AppConfig.COLUMNS.SWEDISH],
            translation: item[AppConfig.COLUMNS.ARABIC],
            example: item[AppConfig.COLUMNS.EXAMPLE_SWE],
            explanation: item[AppConfig.COLUMNS.DEFINITION],
            tags: ['Dagens Ord', 'كلمة اليوم'],
            rawType: item[AppConfig.COLUMNS.TYPE] 
        };
    }
}
