...
// styles are global in layout, but can import here if needed. 
// Standard CSS import from src/styles
      ...
      <div style={styles.modalContent}>
        ...
        <div className="settings-scroll-area">
            ...
            <div style={{ marginTop: '15px' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                {['#3b82f6', '#10b981', '#f59e0b', '#8b5cf6', '#f43f5e'].map(c => (
                  <button key={c} onClick={() => handleAccentChange(c)} style={{ width: '35px', height: '35px', borderRadius: '10px', background: c, border: accentColor === c ? '3px solid #fff' : 'none', cursor: 'pointer' }} />
                ))}
              </div>
            </div>
          </Section>
          ...
          <Section id="info" icon="ℹ️" titleSv="Information" titleAr="معلومات">
            <button onClick={() => router.push('/settings/changelog')} style={{ ...styles.button, width: '100%', marginBottom: '8px', textAlign: 'left' }}>📋 Changelog / سجل التحديثات</button>
            <button onClick={() => router.push('/settings/device-info')} style={{ ...styles.button, width: '100%', textAlign: 'left' }}>🖥️ Enhetsinfo / معلومات الجهاز</button>
          </Section>
